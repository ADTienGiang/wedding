    ...
    <div>
      ...
      <div v-for="(layout, index) in layouts" :key="index" class="image-layout">
        ...
        <div class="vertical-container">
          <div class="image-preview" @click="() => handleImageClick(index, 'vertical1')">
            <img :src="layout.vertical1" alt="Uploaded Image" class="uploaded-image" v-if="layout.vertical1"/>
            <div v-else class="image-placeholder">630x420</div>
          </div>
          <div class="image-preview" @click="() => handleImageClick(index, 'vertical2')">
            <img :src="layout.vertical2" alt="Uploaded Image" class="uploaded-image" v-if="layout.vertical2"/>
            <div v-else class="image-placeholder">630x420</div>
          </div>
          <div class="image-preview" @click="() => handleImageClick(index, 'vertical3')">
            <img :src="layout.vertical3" alt="Uploaded Image" class="uploaded-image" v-if="layout.vertical3"/>
            <div v-else class="image-placeholder">630x420</div>
          </div>
        </div>
        <!-- Container for horizontal images -->
        <div class="horizontal-container">
          <div class="image-preview" @click="() => handleImageClick(index, 'horizontal1')">
            <img :src="layout.horizontal1" alt="Uploaded Image" class="uploaded-image" v-if="layout.horizontal1"/>
            <div v-else class="image-placeholder">420x635</div>
          </div>
          <div class="image-preview" @click="() => handleImageClick(index, 'horizontal2')">
            <img :src="layout.horizontal2" alt="Uploaded Image" class="uploaded-image" v-if="layout.horizontal2"/>
            <div v-else class="image-placeholder">420x635</div>
          </div>
        </div>
      </div>
...
.image-placeholder {
  width: 100%;
  height: auto;
  padding: 10%; /* Adjust padding to maintain aspect ratio */
}

.vertical-container .image-preview {
  flex: 1 1 30%;
  max-width: 30%;
  height: 200px; /* Adjust height for vertical images */
}

.horizontal-container .image-preview {
  flex: 1 1 48%;
  max-width: 48%;
  height: 100px; /* Adjust height for horizontal images */
}
</style>
